...
一个用Go语言编写的现代化桌面应用，支持多种 AI CLI 工具（Claude Code、Codex、Gemini CLI、OpenClaw）的环境变量配置管理。本工具采用现代 Bento Grid 设计风格，使用Wails框架构建，提供简洁优雅的用户界面。
...
- 🔄 **多 Provider 支持** - 支持 Claude、Codex、Gemini CLI、OpenClaw 四种工具
...
#### 📋 配置管理中心
- **多 Provider 支持** - 支持 Claude、Codex、Gemini CLI、OpenClaw 四种工具的配置管理
- **配置列表** - 以卡片网格形式展示所有已保存的配置
- **拖拽排序** - 通过拖拽卡片自由调整配置顺序
- **智能筛选** - 按 Provider 类型筛选配置（All/Claude/Codex/Gemini/OpenClaw）
- **实时搜索** - 通过配置名称快速搜索定位
...
#### 添加新配置
1. 点击左侧面板的 **"新建配置"** 按钮
2. 选择 Provider 类型（Claude / Codex / Gemini CLI / OpenClaw）
3. 根据选择的 Provider 填写相应字段：
   ...
   **OpenClaw 配置**：
   - 配置名称
   - Gateway Base URL
   - Primary/Fallback/Image/PDF Model
   - Skills 相关项（`allowBundled` / `load.extraDirs` / `install.nodeManager` / `load.watch` / `watchDebounceMs`）
   - 路径覆盖（`OPENCLAW_HOME` / `OPENCLAW_STATE_DIR` / `OPENCLAW_CONFIG_PATH`）
   - 可选：自定义 `openclaw.json` 模板
...
```json
{
  ...
  "current_env_openclaw": "OpenClaw Dev",
  "environments": [
    {
      "name": "Development",
    ...
    {
      "name": "OpenClaw Dev",
      "description": "OpenClaw 开发环境",
      "provider": "openclaw",
      "variables": {
        "OPENCLAW_GATEWAY_BASE_URL": "https://your-openclaw-gateway/v1",
        "OPENCLAW_PRIMARY_MODEL": "openai/gpt-4.1",
        "OPENCLAW_FALLBACK_MODELS": "openai/gpt-4o",
        "OPENCLAW_SKILLS_NODE_MANAGER": "pnpm",
        "OPENCLAW_SKILLS_WATCH": "true",
        "OPENCLAW_SKILLS_WATCH_DEBOUNCE_MS": "250"
      }
    }
  ]
}
...
### OpenClaw 配置
生成 OpenClaw 所需配置文件（默认 `~/.openclaw/openclaw.json`，支持路径覆盖）：
- **OPENCLAW_GATEWAY_BASE_URL**: Gateway/OpenAI 兼容 Base URL
- **OPENCLAW_PRIMARY_MODEL**: 主模型
- **OPENCLAW_FALLBACK_MODELS**: 回退模型列表（支持多行）
- **OPENCLAW_IMAGE_MODEL / OPENCLAW_PDF_MODEL**: 图像/PDF 模型
- **OPENCLAW_SKILLS_ALLOW_BUNDLED**: bundled skills 白名单（多行）
- **OPENCLAW_SKILLS_EXTRA_DIRS**: 额外 skills 目录（多行）
- **OPENCLAW_SKILLS_NODE_MANAGER**: skills 安装器（pnpm/npm/yarn/bun）
- **OPENCLAW_SKILLS_WATCH / OPENCLAW_SKILLS_WATCH_DEBOUNCE_MS**: skills watcher 配置
- **OPENCLAW_HOME / OPENCLAW_STATE_DIR / OPENCLAW_CONFIG_PATH**: 路径覆盖
- **自定义模板**: 支持自定义 `openclaw.json`
...
A: 可以！每个 Provider 独立管理，可以同时激活 Claude、Codex、Gemini、OpenClaw 的不同配置。
...
### v1.0.6 (2026-03-08)
**🚀 新增与增强**
- 新增 OpenClaw Provider 全链路支持（配置创建、应用、切换、清理、状态展示）
- OpenClaw 路径解析增强：支持 `OPENCLAW_HOME / OPENCLAW_STATE_DIR / OPENCLAW_CONFIG_PATH`，并兼容 `~`、`$VAR`、`%VAR%`
- OpenClaw 配置读写增强：支持 JSON5 解析；默认写入采用“合并写入”策略，尽量保留已有配置字段
- Skills 平台新增 OpenClaw：支持同步到 `~/.openclaw/skills`（并遵循 `OPENCLAW_STATE_DIR`）
- 配置表单交互优化：OpenClaw 部分枚举/布尔项改为统一风格按钮组；`textarea` 统一增强拉伸体验

**🔧 修复**
- 修复 `configService.ts` 中 Wails 模型 `provider: string` 与前端 `Provider` 类型不匹配导致的 TS 报错
- 修复 MCP 编辑弹窗在部分场景下二次打开表单为空的问题（切换分类后复现概率场景）

**📌 兼容说明**
- MCP 在本工具内已支持 `openclaw` 平台标记与管理；但 OpenClaw 官方当前文档未给出稳定的原生 MCP 配置键位，默认不会强行写入未知 MCP 字段到 `openclaw.json`，以避免配置校验失败。
